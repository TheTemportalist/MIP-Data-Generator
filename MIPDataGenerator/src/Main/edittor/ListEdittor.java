	...
	public Object[] buttonPress(int id, Object[] data) {
		...
		if (id == this.add.id) {
			...
			if (this.getList().getSelectedIndex() >= 0) {
				...
				if (titleFrameText.length() > 100) {
					titleFrameText = "a " + this.getName();
				}
				JFrame frame = new JFrame("Editing: " + titleFrameText);
				...
				if (name.equalsIgnoreCase("Authors")) {
					...
					frame.add(new BlockEdittor(600, 400, this.owner, this.getName(), this.getList()
							.getSelectedIndex(), this.getList().getSelectedValue()));
				}
				else if (name.equalsIgnoreCase("Drops")) {
					frame.add(new DropEdittor(600, 400, this.owner, this.getName(), this.getList()
							.getSelectedIndex(), this.getList().getSelectedValue()));
				}
				else if (name.equalsIgnoreCase("Spawn Biomes")) {
					frame.add(new BiomeEdittor(600, 400, this.owner, this.getName(), this.getList()
							.getSelectedIndex(), this.getList().getSelectedValue()));
				}
